...
var rocmAcct = flag.Bool(
	"rocm-acct",
	false,
	"Enable AMD ROCm GPU device telemetry (requires amd-smi or rocm-smi on this host)")

var rocmSMICmd = flag.String(
	"rocm-smi-cmd",
	"amd-smi",
	"Command used to query ROCm GPU telemetry, e.g. amd-smi or rocm-smi")
...
func main() {
	...
	// Turn on AMD ROCm GPU telemetry only if the corresponding command line option is set to true.
	if *rocmAcct {
		prometheus.MustRegister(NewROCmCollector(*rocmSMICmd)) // from rocm.go
	}
	...
	log.Infof("ROCm GPU Telemetry: %t", *rocmAcct)
